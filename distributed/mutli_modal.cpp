   ...
   while(true)
     {
        ...
        send_to_vdms(vdms_server2, 55561, (receiver_image_1->Receive(topic_image_1,CYAN))->str());
        sender_desc->Send( msg_Desc,topic_desc_1, BLUE);
        send_to_vdms(vdms_server2, 55561, (receiver_desc->Receive(topic_desc_1, GREEN ))->str());
        sender_meta->Send( msg_meta,topic_meta_1, BLUE);
        send_to_vdms(vdms_server2, 55561,(receiver_meta->Receive(topic_meta_1, GREEN ))->str());
           a++;
