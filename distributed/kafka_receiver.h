 ...
 public:
  ...
  // std::stringstream
    ...
    if (topics_.find(topic_str) == topics_.end()) {
      ...
      RdKafka::ErrorCode resp = consumer_->start(topics_[topic_str].get(), 0,RdKafka::Topic::OFFSET_BEGINNING);

      
      
      if (resp != RdKafka::ERR_NO_ERROR) {
        LOG(INFO) << resp << " \t Kafka consume failed: " << RdKafka::err2str(resp);
      }
    }
    ...
    while (true) {
      ...
      if (msg->err() == RdKafka::ERR_NO_ERROR) {
        ...
        // LOG(INFO) << color <<"Kafka reads message at offset " << msg->offset();
        LOG(INFO) << color << "Receiver  " << " \treceived " << static_cast<int>(msg->len())
