...
VDMS::Response send_to_vdms( std::string server_url="localhost", int port=55561, std::string msg="")
{   
    std::basic_string<uint8_t> t=  std::basic_string<uint8_t >((const unsigned  char*)msg.data(), msg.length());   
...
std::string query_body( std::string& query, const std::vector<std::string *> blobs = {}){
    VDMS::protobufs::queryMessage proto_query;
    ...
    std::cout << "Sending size " << proto_query.ByteSize() <<"\t" <<msg_image.length() <<std::endl;
...
std::string send_descriptors( bool new_set, std::string& name){
    ...
    std::memcpy((void*) bytes_str->data(), fv_values.data(), fv_values.size() * sizeof(float));
...
{   Json::Value person_json, bounding_box, add_bounding_box, add_FV_entity,
    ...
    person_json["_ref"] = 1; // to assure the differences between the used references in the DB
