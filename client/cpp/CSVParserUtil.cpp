...
VDMS::CSVParserUtil::CSVParserUtil() : vdms_server("localhost"), vdms_port(55558),
                                       vdms_client(std::unique_ptr<VDMS::VDMSClient>(new VDMS::VDMSClient(vdms_server, vdms_port)))
{
    initCommandsMap();
}

VDMS::CSVParserUtil::CSVParserUtil(const std::string &server, int port, const std::vector<string> columnNames, int index) : vdms_server(server),
                                                                                                                            vdms_port(port),
                                                                                                                            _columnNames(columnNames),
                                                                                                                            id(index), vdms_client(std::unique_ptr<VDMS::VDMSClient>(new VDMS::VDMSClient(vdms_server, vdms_port)))
{
    initCommandsMap();
}
...
void VDMS::CSVParserUtil::parseConstraints(const string &columnNames, const string &row, string &queryType, Json::Value &aquery)
{
...
void VDMS::CSVParserUtil::splitRowOnComma(const std::string &row, std::vector<std::string> &rowvec)
{
