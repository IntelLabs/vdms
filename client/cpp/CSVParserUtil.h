...
class CSVParserUtil {
    ...
    public:
    CSVParserUtil();
    CSVParserUtil(const std::string&, int port, const std::vector<string>, int id );
    ...
    void splitRowOnComma(const std::string& row, std::vector<std::string>& rowvec);
    ...
    VDMS::Response send_to_vdms(const Json::Value &json_query, const std::vector<std::string *> blobs = {});
